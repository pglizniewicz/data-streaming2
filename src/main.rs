...
async fn main() -> std::io::Result<()> {
    ...
    let to_inline_opts: String = opts.symbols;
    ...
    while let Some(_) = interval.next().await {
        ...
        let reqs = to_inline_opts.split(',')
            .map(|symbol| fetch_closing_data(&symbol, &from, &to));

        for result in join_all(reqs).await {
            if result.is_err() {
                println!("Skipping");
                continue;
            }
            let symbol_and_closes = result.unwrap();
            let closes = symbol_and_closes.1;
            if !closes.is_empty() {
